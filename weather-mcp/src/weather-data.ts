...
const FAVORITE_LOCATIONS: Location[] = [
...
export function searchLocations(query: string): Location[] {
  ...
  return FAVORITE_LOCATIONS.filter(loc =>
    loc.city.toLowerCase().includes(lowerQuery) ||
    loc.country.toLowerCase().includes(lowerQuery)
  );
}

export function getCurrentWeather(location: string): CurrentWeather {
...
export function getForecast(location: string, days: number = 7): ForecastDay[] {
...
export function getWeatherAlerts(location: string): WeatherAlert[] {
  ...
  if (Math.random() > 0.7) {
    ...
    const alertTypes = [
      { event: 'Thunderstorm Warning', severity: 'severe' as const, headline: 'Severe thunderstorms expected' },
      { event: 'Heat Advisory', severity: 'moderate' as const, headline: 'Excessive heat expected' },
      { event: 'Winter Storm Watch', severity: 'severe' as const, headline: 'Heavy snow possible' },
      { event: 'Flood Warning', severity: 'extreme' as const, headline: 'Flooding imminent' },
      { event: 'High Wind Warning', severity: 'moderate' as const, headline: 'Damaging winds expected' },
    ];
...
export function getAirQuality(location: string): AirQuality {
  const loc = searchLocations(location)[0] || FAVORITE_LOCATIONS[0];
...
export function getHistoricalWeather(location: string, daysBack: number = 30): any[] {
