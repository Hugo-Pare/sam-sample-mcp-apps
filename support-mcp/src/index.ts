...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    switch (name) {
      case 'list_tickets': {
        const tickets = listTickets(args.status as any, args.assignedTo as string);
        return { content: [{ type: 'text', text: JSON.stringify(tickets, null, 2) }] };
      }

      case 'get_ticket': {
        const ticket = getTicket(args.ticketId as string);
      ...
      case 'update_ticket': {
        const ticket = updateTicket(args.ticketId as string, args.updates as any);
        return { content: [{ type: 'text', text: JSON.stringify(ticket, null, 2) }] };
      }

      case 'assign_ticket': {
        const ticket = assignTicket(args.ticketId as string, args.assignedTo as string);
        return { content: [{ type: 'text', text: JSON.stringify(ticket, null, 2) }] };
      }

      case 'close_ticket': {
        const ticket = closeTicket(args.ticketId as string, args.resolution as string);
        return { content: [{ type: 'text', text: JSON.stringify(ticket, null, 2) }] };
      }

      case 'search_knowledge_base': {
        const articles = searchKnowledgeBase(args.query as string);
        return { content: [{ type: 'text', text: JSON.stringify(articles, null, 2) }] };
      }

      case 'get_customer_info': {
        const customer = getCustomerInfo(args.customerId as string);
        if (!customer) throw new Error(`Customer not found: ${args.customerId}`);
        return { content: [{ type: 'text', text: JSON.stringify(customer, null, 2) }] };
      }

      case 'get_support_stats': {
        const stats = getSupportStats(args.period as string);
      ...
      case 'manage_agents': {
        ...
        let result: any;
        if (action === 'list') result = listAgents();
        else if (action === 'add') result = { success: true, message: `Agent ${args.agentId} added` };
        else if (action === 'remove') result = { success: true, message: `Agent ${args.agentId} removed` };
