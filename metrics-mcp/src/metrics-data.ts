...
const PREDEFINED_DASHBOARDS: Dashboard[] = [
  {
    id: 'dash_revenue_001',
...
const METRIC_DEFINITIONS: Record<string, { name: string; category: MetricCategory; unit: string }> = {
...
export function getAllDashboards(category?: string): Dashboard[] {
  if (category) {
...
export function createDashboard(name: string, widgets: Widget[], createdBy: string): Dashboard {
...
export function getMetric(metricId: string, timeRange?: string): Metric {
...
export function executeQuery(query: string, parameters?: Record<string, any>): QueryResult {
...
export function generateReport(reportType: string, dateRange: { start: string; end: string }): any {
...
export function getSampleDataset(): any {
  return {
...
export function exportData(metricIds: string[], format: ExportFormat): ExportResult {
  ...
  switch (format) {
    case 'csv':
      ...
      const csvRows = metrics
        .map(
          (m) =>
            `${m.id},${m.name},${m.category},${m.value},${m.unit},${m.change},${m.trend},${m.timestamp}`
        )
        .join('\n');
...
export function createUser(name: string, role: string): UserInfo {
  return {
...
export function revokeUser(apiKey: string): { success: boolean; message: string } {
  return {
    success: true,
