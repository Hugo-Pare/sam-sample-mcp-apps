...
const MOCK_PROFILES: Map<string, CustomerProfile> = new Map([
  [
    'user_basic',
    {
      userId: 'user_basic',
...
export function getAccountsByUserId(userId: string): BankAccount[] {
  return MOCK_ACCOUNTS.filter((acc) => acc.userId === userId);
}

export function getAccountBalance(accountId: string, userId: string): BankAccount {
...
export function getTransactions(accountId: string, userId: string, limit: number = 50): Transaction[] {
...
export function getAllTransactionsForUser(userId: string, limit: number = 50): Transaction[] {
...
export function getPaymentHistory(userId: string, limit: number = 50): Payment[] {
  // Get all account IDs for this user
  const userAccountIds = MOCK_ACCOUNTS.filter((acc) => acc.userId === userId).map((acc) => acc.accountId);
