...
app.get('/oauth/authorize', (req, res) => {
  ...
  const requestedScopes = (scopeString as string).split(' ').filter(Boolean);
...
app.post('/oauth/authorize', (req, res) => {
  const {
    ...
    user_id,
    approve,
    scopes, // Selected scopes from checkboxes
  } = req.body;
  ...
  const grantedScopes = selectedScopes.filter((s: string) => user.allowedScopes.includes(s));
