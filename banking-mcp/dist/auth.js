...
export class OAuthError extends Error {
    errorCode;
    statusCode;
    constructor(errorCode, message, statusCode = 401) {
        super(message);
...
export function checkScopePermission(requiredScopes, userScopes) {
    ...
    if (!hasPermission) {
        throw new ForbiddenError(`Insufficient scope. Required: ${requiredScopes.join(' or ')}. Your scopes: ${userScopes.join(' ')}`);
    }
}
// Check tool permission (scope-based)
export function checkToolPermission(toolName, userScopes) {
...
export function checkResourcePermission(uri, userScopes) {
